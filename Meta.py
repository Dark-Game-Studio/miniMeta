"""
Meta: a physics-inspired metaheuristic optimizer in pure Python.

Mathematical concepts:
  - gradient (analytic or finite-difference)
  - vector algebra (addition, scaling, norm)
  - momentum and damping in discrete time

Physical principles:
  - Newton's second law: F = m * a
  - Stokes-like friction: force opposing velocity
  - thermal noise (Brownian motion) for exploration
  - annealing: cooling of temperature to reduce noise over time
"""
...
class Meta:
    """
    A metaheuristic optimizer that treats the objective function as a
    potential energy landscape.  A single fictitious particle moves
    according to Newton's second law with damping and thermal noise.
    ...
    With damping (friction coefficient γ) and temperature T:
      v' = γ * v + a*dt
      x' = x + v'*dt
      Additionally, a random force √(2γ kT) * N(0,1) is added to mimic a
      heat bath (fluctuation–dissipation theorem).  For simplicity we use
      a Gaussian noise scaled by current temperature.

    Parameters:
      dim             - search space dimensionality
      objective       - function f(x) -> float (potential energy)
      grad            - analytic gradient; if None, finite differences are used
      mass            - particle mass (scales acceleration)
      damping         - velocity retention factor (0=no inertia, 1=frictionless)
      temperature     - initial 'thermal energy' (noise scale)
      cooling_rate    - multiplicative factor per step (annealing)
      dt              - time step for integration (usually 1.0)
    """
    def __init__(self,
                 dim: int,
